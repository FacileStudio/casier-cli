...
const CLEF_TOML: &str = ".clef.toml";

pub async fn run() -> Result<()> {
    let target = PathBuf::from(CLEF_TOML);
    if target.exists() {
        bail!("{} already exists in this directory", CLEF_TOML);
    }
...
"#,
    ...
    std::fs::write(&target, &content)
        .with_context(|| format!("failed to write {}", CLEF_TOML))?;

    println!("Created {} (space={}, env={})", CLEF_TOML, space.slug, env);
    Ok(())
}
