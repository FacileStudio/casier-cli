...
pub async fn run(project: &str, env_a: &str, env_b: &str) -> Result<()> {
    ...
    cache::store(project, env_b, &cache::to_map(&secrets_b));

    let map_a: BTreeMap<&str, &str> = secrets_a
        .iter()
        .map(|s| (s.key.as_str(), s.value.as_str()))
        .collect();
    let map_b: BTreeMap<&str, &str> = secrets_b
        .iter()
        .map(|s| (s.key.as_str(), s.value.as_str()))
        .collect();
