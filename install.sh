...
REPO="https://github.com/FacileStudio/casier-cli.git"
BIN_NAME="casier"

info()  { printf '\033[1;36m%s\033[0m\n' "$*"; }
error() { printf '\033[1;31merror:\033[0m %s\n' "$*" >&2; exit 1; }

SKILL_MARKER_START="<!-- casier:start -->"
SKILL_MARKER_END="<!-- casier:end -->"
...
register_skill() {
  ...
  if command -v claude &>/dev/null; then
    local skill_dir="$HOME/.claude/skills/casier"
    mkdir -p "$skill_dir"
...
info "Cloning $REPO..."
git clone --depth 1 --quiet "$REPO" "$TMPDIR/casier-cli"

info "Building and installing $BIN_NAME..."
cargo install --path "$TMPDIR/casier-cli" --force --quiet

register_skill "$TMPDIR/casier-cli"
